...
export async function handleCallback(ctx: Context) {
  ...
  if (data.startsWith('lang_')) {
    ...
    await ctx.answerCbQuery(t("language_saved", selectedLang), { show_alert: true });
    // @ts-ignore
  ...
  const processingText = t("processing", userLang).replace("{{progress}}", buildProgressBar(0));
  // @ts-ignore
  const processingMsg = await ctx.reply(processingText, { reply_to_message_id: ctx.callbackQuery.message?.reply_to_message?.message_id });
  ...
  const handleProgress = async (percent: number) => {
    ...
    if (now - lastUpdateTime > 2000) {
      lastUpdateTime = now;
      const updatedText = t("processing", userLang).replace("{{progress}}", buildProgressBar(percent));
      try {
        await ctx.telegram.editMessageText(ctx.chat!.id, processingMsg.message_id, undefined, updatedText);
      } catch (err) {
  ...
  try {
    ...
    if (mediaData.type === "audio") {
      processedPath = await processAudio(downloadedPath, quality, handleProgress);
    } else {
      processedPath = await processVideo(downloadedPath, quality, handleProgress);
    }

    // Set final 100% processing bar
    const finalText = t("processing", userLang).replace("{{progress}}", buildProgressBar(100));
    try {
    ...
    const savedPercent = oldBytes > 0 ? ((oldBytes - newBytes) / oldBytes * 100).toFixed(1) : "0.0";
    ...
    try {
      await ctx.telegram.editMessageText(ctx.chat!.id, processingMsg.message_id, undefined, t("uploading", userLang));
    } catch(err) {}

    if (mediaData.type === "audio") {
      await ctx.replyWithAudio({ source: processedPath }, { caption: finalReport });
    } else {
      await ctx.replyWithVideo({ source: processedPath }, { caption: finalReport });
    }
