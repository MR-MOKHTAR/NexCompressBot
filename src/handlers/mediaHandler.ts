...
import { getAudioKeyboard, getVideoKeyboard } from "../keyboards/qualityKeyboard";
...
export async function handleAudio(ctx: Context) {
  ...
  const shortId = saveMedia(fileId, "audio");
...
export async function handleVideo(ctx: Context) {
  ...
  // @ts-ignore
  const videoObj = ctx.message?.video || ctx.message?.video_note || ctx.message?.document;
  ...
  const shortId = saveMedia(fileId, "video");
